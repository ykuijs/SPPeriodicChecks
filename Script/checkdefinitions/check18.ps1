...
{
    $sb = {
        Write-Log "Starting Check 18: Distributed Cache Status check"
        $results.Check18 = ""
        ...
        {
            if ($null -eq (Get-Module DistributedCacheAdministration -ListAvailable))
            {
                if (Test-Path -Path "C:\Program Files\AppFabric 1.1 for Windows Server\PowershellModules\DistributedCacheAdministration")
                {
                    Import-Module "C:\Program Files\AppFabric 1.1 for Windows Server\PowershellModules\DistributedCacheAdministration"
                }
                else
                {
                    $errorCount++
        ...
        {
            Write-Log "Skipping check. Distributed Cache service not running on this server."
        }
        ...
        Write-Log "Completed Check 18: Distributed Cache Status check"
    }
